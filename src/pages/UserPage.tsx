...
type User = {
  ...
  phone: string;
  role: "ADMIN" | "CUSTOMER";
  deleted: string; // e.g., "Not Deleted"
...
export default function UserManagementPage() {
  ...
  const fetchUsers = async (
    ...
    try {
      const response = await api.users.getAll({
        page: resetPage ? 0 : currentPage, // Use the first page if resetPage is true
        size: 8,
        sort: "id",
        search, // Pass the search parameter to the API
      });
  ...
  const handleEditUser = async (userId: number) => {
    try {
      const response = await api.users.getById(userId); // Fetch user details
  ...
  const handleRestoreUser = async (userId: number) => {
    try {
      ...
      // Update the user's "deleted" status in the list
      setUsers((prevUsers) =>
        prevUsers.map((user) =>
                      ...
                      {confirmingDeleteUserId === user.id ? (
                        ...
                        <div className="flex items-center space-x-2">
                          <button
                            ...
                            disabled={confirmingDeleteUserId !== null} // Disable edit while confirming delete
                          >
                            Edit
                          </button>
